...
def plot_overview_dashboard(round_data, save_path=None):
    ...
    fig.suptitle(
        "BATTLE OF MYTROS — CAMPAIGN OVERVIEW", fontsize=18, fontweight="bold", color=GOLD, y=0.97
    )
    ...
    ax.bar(
        x - w / 2,
        round_data["allied_total_injuries"],
        w,
        color=ALLIED_COLOR,
        alpha=0.85,
        label="Allied",
    ...
    ax.bar(
        x + w / 2,
        round_data["enemy_total_injuries"],
        w,
        color=ENEMY_COLOR,
        alpha=0.85,
        label="Enemy",
    ...
    ax.plot(
        rounds,
        round_data["allied_commanders_alive"],
        "-o",
        color=ALLIED_COLOR,
        linewidth=2.5,
        markersize=6,
    ...
    ax.plot(
        rounds,
        round_data["enemy_commanders_alive"],
        "-s",
        color=ENEMY_COLOR,
        linewidth=2.5,
        markersize=6,
...
def plot_legion_detail(allied, enemy, save_path=None):
    ...
    active = [legion for legion in all_legions if legion.history_injuries]
    if not active:
        return None

    max_rounds = max(len(legion.history_injuries) for legion in active)
    fig, axes = plt.subplots(1, 2, figsize=(18, max(6, len(active) * 0.45 + 1.5)))
    fig.suptitle(
        "LEGION STATUS — INJURIES & MORALE PER ROUND",
        fontsize=16,
        fontweight="bold",
        color=GOLD,
        y=0.98,
    ...
    allied_sorted = sorted(
        [legion for legion in allied if legion.history_injuries], key=lambda legion: legion.name
    )
    enemy_sorted = sorted(
        [legion for legion in enemy if legion.history_injuries], key=lambda legion: legion.name
    )
    ordered = allied_sorted + enemy_sorted
    names = []
    for legion in ordered:
        prefix = "[A]" if legion.faction == Faction.ALLIED else "[E]"
        suffix = (
            " [DESTROYED]"
            if legion.destroyed
            else (" [Cmdr Dead]" if not legion.commander.alive else "")
        )
        names.append(f"{prefix} {legion.name}{suffix}")
    ...
    for ax_idx, (data_attr, cmap, vmin, vmax, title) in enumerate(
        ...
        for i, legion in enumerate(ordered):
            for j, val in enumerate(getattr(legion, data_attr)):
                data_arr[i, j] = val

        im = ax.imshow(
            data_arr, cmap=cmap, aspect="auto", vmin=vmin, vmax=vmax, interpolation="nearest"
        )
        ...
        for i in range(len(ordered)):
            for j in range(min(len(getattr(ordered[i], data_attr)), max_rounds)):
                ...
                color = (
                    "white" if (ax_idx == 0 and val >= 3) or (ax_idx == 1 and val <= 2) else "black"
                )
                ax.text(
                    j,
                    i,
...
def plot_battle_results(summaries, save_path=None):
    setup_style()
    if not any(s.battles for s in summaries):
        return None
    ...
    ax.legend(
        handles=[
            mpatches.Patch(color=ALLIED_COLOR, label="Allied Victory"),
...
def plot_commander_status(allied, enemy, save_path=None):
    ...
    setup_style()
    all_commanders = [
        (legion.name, legion.commander, Faction.ALLIED, legion.injuries, legion.destroyed)
        for legion in allied
    ] + [
        (legion.name, legion.commander, Faction.ENEMY, legion.injuries, legion.destroyed)
        for legion in enemy
    ]
    ...
    for i, (legion_name, cmdr, faction, injuries, destroyed) in enumerate(reversed(all_commanders)):
        ...
        ax.text(
            -0.05,
            y,
            f"{faction_marker} {cmdr.name}  [{legion_name}]",
            ha="right",
            va="center",
            fontsize=8.5,
            fontweight="bold",
        ...
        ax.text(
            1.1,
            y,
            f"{status}  |  {injuries} inj  |  {tags_str}",
            ha="left",
            va="center",
            fontsize=7.5,
            color=status_color,
...
def plot_stat_radar(allied, enemy, save_path=None):
    ...
    allied_active = [legion for legion in allied if not legion.destroyed]
    enemy_active = [legion for legion in enemy if not legion.destroyed]

    def avg(legions, attr):
        vals = [getattr(legion, attr + "_total") for legion in legions]
    ...
    allied_vals = [
        ...
        avg(allied, "mor"),
        avg(allied, "wit"),
        max(0, MAX_INJURIES - np.mean([legion.injuries for legion in allied_active]))
        if allied_active
        else 0,
        np.mean([legion.mor_total for legion in allied_active]) if allied_active else 0,
        sum(1 for legion in allied if legion.commander.alive) / len(allied) * 6,
    ]
    enemy_vals = [
        avg(enemy, "vit"),
        avg(enemy, "mor"),
        avg(enemy, "wit"),
        max(0, MAX_INJURIES - np.mean([legion.injuries for legion in enemy_active]))
        if enemy_active
        else 0,
        np.mean([legion.mor_total for legion in enemy_active]) if enemy_active else 0,
        sum(1 for legion in enemy if legion.commander.alive) / len(enemy) * 6,
    ]
    ...
    ax.plot(
        angles, allied_norm, "o-", color=ALLIED_COLOR, linewidth=2, label="Allied", markersize=6
    )
    ...
    ax.set_title(
        "ARMY STRENGTH COMPARISON", fontsize=16, fontweight="bold", color=GOLD, pad=25, y=1.08
    )
    ax.legend(
        loc="upper right",
...
def plot_morale_timeline(allied, enemy, save_path=None):
    setup_style()
    legions_with_history = [legion for legion in allied + enemy if legion.history_mor]
    ...
    fig.suptitle(
        "MORALE TIMELINE — ALL LEGIONS", fontsize=16, fontweight="bold", color=GOLD, y=0.98
    )
    ...
    for idx, legion in enumerate(legions_with_history):
        ...
        ax.set_title(
            f"{faction_t} {legion.name} ({status}, ☠{legion.commanders_lost})",
            fontsize=8,
            fontweight="bold",
            color=light,
            pad=3,
...
def plot_graveyard(allied, enemy, summaries, save_path=None):
    ...
    fig.suptitle(
        "BATTLE GRAVEYARD — LOSSES OVER TIME", fontsize=16, fontweight="bold", color=GOLD, y=0.97
    )

    ax = axes[0]
    all_legions = sorted(
        allied + enemy,
        key=lambda legion: (0 if legion.destroyed else 1, len(legion.history_results)),
    )
    y_labels = []
    for y, legion in enumerate(all_legions):
        ...
        if legion.commanders_lost > 0:
            skull = "☠" * min(legion.commanders_lost, 5) + (
                "…" if legion.commanders_lost > 5 else ""
            )
            ax.text(rnds + 0.3, y, skull, va="center", fontsize=7, color=ENEMY_LIGHT)
        status = (
            f"[DEAD] R{rnds}" if legion.destroyed else ("[ROUTED]" if legion.routed else "[OK]")
        )
        ax.text(
            0.3, y, status, va="center", ha="left", fontsize=7, fontweight="bold", color="white"
        )
        faction_t = "[A]" if legion.faction == Faction.ALLIED else "[E]"
        y_labels.append(f"{faction_t} {legion.name}")

    ax.set_yticks(range(len(all_legions)))
    ...
    for s in summaries:
        ...
        ae = len(s.enemy_commander_deaths)
        et += ae
        a_cum.append(at)
        e_cum.append(et)
    ...
    ax2.plot(
        rl, e_cum, "-s", color=ENEMY_COLOR, linewidth=2, markersize=3, label=f"Enemy (total: {et})"
    )
...
def plot_death_toll(round_data, save_path=None):
    ...
    fig.suptitle(
        "CIVILIAN DEATH TOLL — BATTLE OF MYTROS", fontsize=16, fontweight="bold", color=GOLD, y=0.98
    )
    ...
    ax.set_title(
        "Civilian Deaths per Round", fontsize=12, fontweight="bold", color=TEXT_COLOR, pad=10
    )
    ax.set_xlabel("Round")
    ...
    for bar, val in zip(bars, per_rnd):
        if val > 0:
            ax.text(
                ...
                bar.get_height() + max(per_rnd) * 0.01,
                f"{val:,}",
                ha="center",
                va="bottom",
                fontsize=7,
    ...
    ax.set_title(
        f"Cumulative Civilian Deaths  (total: {total:,})",
        fontsize=12,
        fontweight="bold",
        color=TEXT_COLOR,
        pad=10,
...
def plot_balance_analysis(
    legions_path="legions.csv", commanders_path="commanders.csv", save_path=None
):
    setup_style()
    ...
    def row(legion):
        return {
            ...
            "mor": legion.mor,
            "wit": legion.wit,
            "total": legion.vit + legion.mor + legion.wit,
            "cmd": legion.commander.name,
            "tags": legion.commander.tags,
        }

    a_rows = [row(legion) for legion in allied]
    e_rows = [row(legion) for legion in enemy]

    fig = plt.figure(figsize=(22, 14))
    fig.suptitle(
        "BALANCE ANALYSIS — LEGION STATS", fontsize=18, fontweight="bold", color=GOLD, y=0.98
    )
    ...
    is_allied = [any(legion.name == r["name"] for legion in allied) for r in all_rows]
    ...
    for i, r in enumerate(all_rows):
        ...
        ax1.text(
            ...
            va="top",
            fontsize=6,
            color=ALLIED_LIGHT if is_allied[i] else ENEMY_LIGHT,
            rotation=45,
        )

    ax1.set_xticks(x)
    ax1.set_xticklabels(names, rotation=40, ha="right", fontsize=8)
    ax1.set_ylabel("Combined Stat Total")
    ax1.legend(facecolor=CARD_COLOR, edgecolor=GRID_COLOR, fontsize=9)
    ax1.axhline(
        np.mean([r["total"] for r in a_rows]),
        color=ALLIED_LIGHT,
        linestyle="--",
        linewidth=1.5,
        label="Allied avg",
    ...
    ax1.axhline(
        np.mean([r["total"] for r in e_rows]),
        color=ENEMY_LIGHT,
        linestyle="--",
        linewidth=1.5,
        label="Enemy avg",
    ...
    ax1.set_title(
        "Total Stats per Legion (Vitality + Morale + Wit)", fontsize=11, fontweight="bold", pad=8
    )
    ...
    ax3.legend(
        handles=[
            mpatches.Patch(color=ALLIED_COLOR, label="Allied"),
...
def plot_monte_carlo(
    ...
    num_rounds=100,
    save_path=None,
    legions_path="legions.csv",
    commanders_path="commanders.csv",
):
    setup_style()
    results = {
        "allied_surviving": [],
    ...
    for i in range(num_sims):
        ...
        for rnd in range(1, num_rounds + 1):
            ...
            eff_a = [legion for legion in al if legion.effective]
            eff_e = [legion for legion in en if legion.effective]
            if not eff_a or not eff_e:
                final_rnd = rnd
                break

        surv_a = sum(1 for legion in al if not legion.destroyed)
        surv_e = sum(1 for legion in en if not legion.destroyed)
        eff_a_c = sum(1 for legion in al if legion.effective)
        eff_e_c = sum(1 for legion in en if legion.effective)
        results["allied_surviving"].append(surv_a)
        results["enemy_surviving"].append(surv_e)
        results["allied_cmdr_deaths"].append(sum(legion.commanders_lost for legion in al))
        results["enemy_cmdr_deaths"].append(sum(legion.commanders_lost for legion in en))
        results["rounds_to_end"].append(final_rnd)
        results["total_civilian_deaths"].append(sim_deaths)
        if eff_a_c > 0 and eff_e_c == 0:
            results["winner"].append("Allied")
        elif eff_e_c > 0 and eff_a_c == 0:
    ...
    fig.suptitle(
        f"MONTE CARLO ANALYSIS — {num_sims} SIMULATIONS",
        fontsize=18,
        fontweight="bold",
        color=GOLD,
        y=0.97,
    ...
    bars = ax.bar(
        ...
        color=[ALLIED_COLOR, ENEMY_COLOR, GOLD],
        alpha=0.85,
        edgecolor="white",
        linewidth=0.5,
    )
    ...
    ax.axvline(
        np.mean(results["allied_surviving"]),
        color=ALLIED_LIGHT,
        linestyle="--",
        linewidth=2,
    ...
    ax.axvline(
        np.mean(results["enemy_surviving"]),
        color=ENEMY_LIGHT,
        linestyle="--",
        linewidth=2,
    ...
    if draw_rnds:
        ax.hist(draw_rnds, bins=bins_r, alpha=0.6, color=GOLD, label="Draws")
    ...
    cmdr_entries = [
        (n, len(rnds) / num_sims)
        for n, rnds in cmdr_death_rounds.items()
    ...
    if cmdr_entries:
        ...
        ax.set_yticks(np.arange(len(names_c)))
    ...
    data_box = [
        results["allied_surviving"],
        results["enemy_surviving"],
        results["allied_cmdr_deaths"],
        results["enemy_cmdr_deaths"],
    ]
    bp = ax.boxplot(
        data_box,
        patch_artist=True,
    ...
    dw, lw, cw = [], [], []
    if allied_rnds:
        dw.append(allied_rnds)
        lw.append(f"Allied\n(n={len(allied_rnds)})")
        cw.append(ALLIED_COLOR)
    if enemy_rnds:
        dw.append(enemy_rnds)
        lw.append(f"Enemy\n(n={len(enemy_rnds)})")
        cw.append(ENEMY_COLOR)
    if draw_rnds:
        dw.append(draw_rnds)
        lw.append(f"Draw\n(n={len(draw_rnds)})")
        cw.append(GOLD)
    if dw:
        bp2 = ax.boxplot(
            dw, patch_artist=True, tick_labels=lw, medianprops=dict(color="white", linewidth=2)
        )
    ...
    ax.set_title(
        "Civilian Deaths per Round (avg across sims)", fontsize=11, fontweight="bold", pad=10
    )
    ax.set_xlabel("Round")
    ax.set_ylabel("Deaths")
    ax.legend(facecolor=CARD_COLOR, edgecolor=GRID_COLOR, fontsize=8)
    ...
    ax = axes[2, 2]
    allied_civ = [
        d for d, w in zip(results["total_civilian_deaths"], results["winner"]) if w == "Allied"
    ]
    enemy_civ = [
        d for d, w in zip(results["total_civilian_deaths"], results["winner"]) if w == "Enemy"
    ]
    draw_civ = [
        d for d, w in zip(results["total_civilian_deaths"], results["winner"]) if w == "Draw"
    ]
    box_data, box_labels, box_colors = [], [], []
    if allied_civ:
        box_data.append(allied_civ)
        box_labels.append("Allied\nwins")
        box_colors.append(ALLIED_COLOR)
    if enemy_civ:
        box_data.append(enemy_civ)
        box_labels.append("Enemy\nwins")
        box_colors.append(ENEMY_COLOR)
    if draw_civ:
        box_data.append(draw_civ)
        box_labels.append("Draw")
        box_colors.append(GOLD)
    if box_data:
        bp3 = ax.boxplot(
            box_data,
            patch_artist=True,
...
def plot_cmdr_deaths_analysis(all_death_events, num_sims, save_path=None):
    ...
    fig.suptitle(
        f"COMMANDER DEATHS ANALYSIS — {num_sims} SIMULATIONS",
        fontsize=18,
        fontweight="bold",
        color=GOLD,
        y=0.97,
    ...
    counts_a_p = [
        sum(1 for e in allied_events if e["protection"] == p) / num_sims for p in prot_levels
    ]
    counts_e_p = [
        sum(1 for e in enemy_events if e["protection"] == p) / num_sims for p in prot_levels
    ]
    ax.bar(
        prot_levels - width / 2, counts_a_p, width, label="Allied", color=ALLIED_COLOR, alpha=0.85
    )
