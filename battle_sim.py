...
BATTLE_COUNTER_WIN = 1             # Counter points scored for winning a battle phase
BATTLE_COUNTER_NAT20_BONUS = 2     # Extra counter points on a natural 20 (added to WIN)
BATTLE_COUNTER_NAT1_PENALTY = 2    # Extra counter points LOST on a natural 1 (added to loss)

# ── Aftermath DCs ──
RECOVERY_BASE_DC = 10              # Base DC for Recovery check; actual DC = this + current injuries
HOPE_DC = 10                       # DC for the Hope (morale) check
...
MORALE_CAP = 12                    # Maximum morale a legion can reach (base + commander + modifiers)
...
# ── Commander Casualty ──
# Commander survives if d20 + Vit bonus + tag bonuses ≥ DC (standard D&D check)
CASUALTY_DC = {"winner": 3, "loser": 5, "crushed": 7}  # "crushed" = lost by 3+ on battle counter
CASUALTY_CRUSHED_THRESHOLD = -3    # Battle counter diff at or below this → use "crushed" DC
CASUALTY_TAG_BONUS = {"Steadfast": 2, "Veteran": 2, "Duelist": 4}  # Tag bonuses to casualty roll
CASUALTY_INJURED_PENALTY = -2      # Penalty when legion has ≥ CASUALTY_INJURED_AT injuries
CASUALTY_INJURED_AT = 5            # Injury threshold for the penalty above
COMMANDER_DEATH_MORALE_LOSS = 1    # Morale lost by the legion when its commander dies

# ── Tag effects (combat) ──
FANATIC_BONUS = 4                  # Bonus to one battle roll when Fanatic is activated
...
ZEALOT_BONUS = 1                   # Bonus to all battle rolls from Zealot
...
RECON_FULL_INTEL = "Full intelligence (movements + actions)"  # result if above all thresholds
...
class Terrain(Enum):
    OPEN = "Open Ground"
    CHOKEPOINT = "Urban Chokepoint"
    RIVER = "River Crossing"
    HIGH_GROUND = "High Ground"
    FORTIFIED = "Fortified Walls"
...
class Legion:
    ...
    @property
    def mor_total(self):
        base = self.mor_base + (self.commander.mor_bonus if self.commander.alive else 0) + self.morale_mod
        return min(MORALE_CAP, max(0, base))
...
def simulate_battle(legion_a: Legion, legion_b: Legion, log: BattleLog):
    ...
    # Tactician: use on the phase matching the legion's best stat
    for legion in [legion_a, legion_b]:
        if legion.commander.alive:
            # Fanatic: use on Clash if vitality is low (desperate push)
            ...
            # Tactician: use advantage on Maneuver
            ...
            # Mage: impose disadvantage on enemy's best phase
            if legion.commander.has_tag("Mage"):
                legion.commander.use_tag("Mage")
                legion._mage_target = "maneuver"  # target enemy maneuver
            else:
    ...
    ra, rb, ta, tb, n20a, n20b, n1a, n1b = contested_roll(
        legion_a.vit_total + clash_bonus_a + fanatic_bonus_a,
        legion_b.vit_total + clash_bonus_b + fanatic_bonus_b
    )
    winner = determine_phase_winner(ta, tb, n20a, n20b, n1a, n1b)
    da, db = 0, 0
    if winner == 'a':
...
def run_aftermath(legion: Legion, won: bool, battle_counter_diff: int):
    ...
    # Steadfast: prevent 1 injury on fail
    steadfast_active = legion.commander.alive and legion.commander.has_tag("Steadfast")

    passed = (roll != 1) and (roll + legion.vit_total >= dc)
    if won:
    ...
    # Brutal enemy tag effect (simulated as disadvantage already handled in battle)
    # Steadfast reduces by 1
    if not passed and steadfast_active:
        injuries_gained = max(0, injuries_gained - 1)
        legion.commander.use_tag("Steadfast")

    # Fanatic injury
    if getattr(legion, '_fanatic_active', False):
        injuries_gained += FANATIC_EXTRA_INJURY

    # Medic: reroll failed recovery or remove 1 injury
    if not passed and legion.commander.alive and legion.commander.has_tag("Medic"):
        reroll = d20()
        if reroll + legion.vit_total >= dc:
            injuries_gained = RECOVERY_WINNER_PASS if won else RECOVERY_LOSER_PASS
            legion.commander.use_tag("Medic")
        else:
            legion.commander.use_tag("Medic")
            injuries_gained = max(0, injuries_gained - 1)  # remove 1 instead
    ...
    # Rallier: auto-succeed
    if not passed and legion.commander.alive and legion.commander.has_tag("Rallier"):
        passed = True
    ...
    # --- Commander Casualty Check (DC-based survival) ---
    if legion.commander.alive:
        roll = d20()
        vit_bonus = legion.commander.vit_bonus
        extra = 0
        for tag_name, tag_bonus in CASUALTY_TAG_BONUS.items():
            if any(t.name == tag_name for t in legion.commander.tag_objects):
                extra += tag_bonus
        if legion.injuries >= CASUALTY_INJURED_AT:
            extra += CASUALTY_INJURED_PENALTY

        effective_roll = roll + vit_bonus + extra

        if won:
            dc = CASUALTY_DC["winner"]
        elif battle_counter_diff <= CASUALTY_CRUSHED_THRESHOLD:
            dc = CASUALTY_DC["crushed"]
        else:
            dc = CASUALTY_DC["loser"]

        survived = (roll != 1) and (effective_roll >= dc)  # nat 1 always fails

        # Duelist second chance
        if not survived and any(t.name == "Duelist" for t in legion.commander.tag_objects):
            roll2 = d20()
            effective_roll2 = roll2 + vit_bonus + extra
            if (roll2 != 1) and (effective_roll2 >= dc):
                survived = True
        ...
        results["casualty"] = {"roll": roll, "effective": effective_roll, "dc": dc, "died": died}
    else:
...
class CommanderPool:
    ...
    def get_replacement(self, faction: Faction) -> Optional[Commander]:
        """Get the best available replacement commander for the faction.
        Picks the commander with the highest total bonus."""
        pool = self.allied_reserves if faction == Faction.ALLIED else self.enemy_reserves
        if not pool:
            return None
        # Sort by total bonus descending, pick best
        pool.sort(key=lambda c: c.total_bonus, reverse=True)
        return pool.pop(0)
...
def build_armies_from_csv(legions_path: str, commanders_path: str):
    ...
    for cdef in commander_defs:
        ...
        if faction is None:
            print(f"  WARNING: Unknown faction '{cdef['faction']}' for commander {cdef['name']}, skipping.")
            continue
        ...
        if cdef['legion']:
            assigned_commanders[cdef['legion']] = cmd
        else:
    ...
    # Build legion objects
    allied = []
    enemy = []

    for ldef in legion_defs:
        if not ldef['active']:
            continue  # skip inactive/reserve legions

        faction_key = ldef['faction'].lower()
        faction = faction_map.get(faction_key)
        if faction is None:
            print(f"  WARNING: Unknown faction '{ldef['faction']}' for legion {ldef['name']}, skipping.")
            continue

        commander = assigned_commanders.get(ldef['name'])
        if commander is None:
            # Try to pull from reserves
            pool = allied_reserves if faction == Faction.ALLIED else enemy_reserves
            if pool:
                pool.sort(key=lambda c: c.total_bonus, reverse=True)
                commander = pool.pop(0)
                print(f"  NOTE: No commander assigned to {ldef['name']}, using reserve: {commander.name}")
            else:
                # Create a placeholder commander with 0 stats
                commander = Commander(name="(Vacant)", vit_bonus=0, mor_bonus=0, wit_bonus=0, tags=[])
                print(f"  WARNING: No commander available for {ldef['name']}!")
...
def plot_monte_carlo(num_sims=200, num_rounds=100, save_path=None,
    ...
    for i in range(num_sims):
        ...
        for rnd in range(1, MAX_ROUNDS + 1):
            simulate_round(allied, enemy, rnd, pool)
    ...
    # Panel 4: Commander deaths distribution
    ax = axes[1, 0]
    bins2 = range(0, max(max(results["allied_cmdr_deaths"]), max(results["enemy_cmdr_deaths"])) + 2)
    ax.hist(results["allied_cmdr_deaths"], bins=bins2, alpha=0.6, color=ALLIED_COLOR,
            label="Allied", edgecolor='white', linewidth=0.5)
    ax.hist(results["enemy_cmdr_deaths"], bins=bins2, alpha=0.6, color=ENEMY_COLOR,
            label="Enemy", edgecolor='white', linewidth=0.5)
    ax.axvline(np.mean(results["allied_cmdr_deaths"]), color=ALLIED_LIGHT, linestyle='--', linewidth=2,
               label=f"Allied avg: {np.mean(results['allied_cmdr_deaths']):.1f}")
    ax.axvline(np.mean(results["enemy_cmdr_deaths"]), color=ENEMY_LIGHT, linestyle='--', linewidth=2,
               label=f"Enemy avg: {np.mean(results['enemy_cmdr_deaths']):.1f}")
    ax.set_title("Commander Deaths Distribution", fontsize=13, fontweight='bold', pad=10)
    ax.set_xlabel("Commanders Killed")
    ax.set_ylabel("Frequency")
    ax.legend(facecolor=CARD_COLOR, edgecolor=GRID_COLOR, fontsize=8)
...
def main():
    ...
    fig7 = plot_morale_timeline(allied, enemy,
                                 save_path=f"{prefix}07_morale_timeline.png" if save else None)
    fig8 = plot_graveyard(allied, enemy, summaries,
                           save_path=f"{prefix}08_graveyard.png" if save else None)

    # Monte Carlo
    if args.monte_carlo > 0:
        fig6 = plot_monte_carlo(args.monte_carlo, args.rounds,
                                save_path=f"{prefix}06_monte_carlo.png" if save else None,
